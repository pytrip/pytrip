...
class LinePlotPanel(wx.Panel):
    ...
    def on_key_down(self,evt):
        if hasattr(self,"nearest_point"):
            ...
            if code in prevkey:
                ...
                data = getattr(self.nearest_point["voi"],self.data_method)(getattr(self.nearest_point["plan"],self.cube_method)())                
    ...
    def draw_nearest_text(self):
        ...
        text = "Vol: " + unicode("%.2f"%(point[0]*100)) + "%%\n%s: "%self.quantity + unicode("%.2f"%(point[1]))+"%s"%self.unit
            ...
            for voi in plan.get_vois():
                if voi.is_plan_selected():
                    ...
                    if data is None:
                        continue
                    n = np.where(data<= 0.0)[0][0] 
                    self.subplot.plot(np.linspace(0,n/10,n),data[0:n]*100,label="%s: %s"%(plan.get_name(),voi.get_name()) ,color=(np.array(voi.get_voi().get_color())/255.0),linestyle=markertypes[i%4])
