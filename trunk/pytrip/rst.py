...
class Rst:
    def __init__(self):
        self.var_dict = {"rstfile":"rstfile","sistable":"sistable","patient_id":"patient_name",
                        ...
                        "#submachines":"submachines","bolus":"bolus","ripplefilter":"ripplefilter","mass":"mass","charge":"charge","#particles":"particles"}
        self.machines = []
...
class SubMachine:
    ...
    def get_raster_grid(self):
        ...
        grid[np.array(rasterpoints[:,0]/self.stepsize[0]+zero[0],'uint8'),np.array(rasterpoints[:,1]/self.stepsize[1]+zero[1],'uint8')] = rasterpoints[:,2]
        return grid
    ...
    def generate_random_error_machine(self,sigma):
        ...
        offset = np.array([[x,y] for x in range(-int(sigma/self.stepsize[0]),int(sigma/self.stepsize[0])+1) for y in range(-int(sigma/self.stepsize[1]),int(sigma/self.stepsize[1])+1)])
