...
def pluginLoader(parent):
    ...
    panelLETPlot = res.LoadPanel(parent, 'pluginLetPlot')
    panelLETPlot.Init(res)

    return panelLETPlot

class pluginLetPlot(wx.Panel):
    ...
    def Init(self, res):
        ...
        pub.subscribe(self.OnRawDataUpdate, 'patient.updated.raw_data')
    ...
    def OnRawDataUpdate(self,msg):
        if msg.data.has_key('letdata'):
            self.letcube = msg.data['letdata']
            self.let_max = self.letcube.get_max()
            self.let_value = self.let_max/2
    ...
    def OnPaint(self, evt):
        ...
        if len(self.images):
            ...
            i_z = self.letcube.pos_to_indices([0,0,float(self.z)])[2]
            grid = self.letcube.cube[i_z]
            x, y = np.meshgrid(
                    np.arange(grid.shape[1]), np.arange(grid.shape[0]))
            isoletgen = cntr.Cntr(x, y, grid)
            self.DrawIsoLET(self.let_value,gc,isoletgen)
    ...
    def DrawIsoLET(self,let,gc,isoletgen):
        contours = isoletgen.trace(let)
        # matplotlib 1.0.0 and above returns vertices and segments, but we only need vertices
        if (mplversion >= "1.0.0"):
            contours = contours[:len(contours)//2]
        if len(contours):
    ...
    def OnUpdatePositionValues(self, evt=None):
        ...
        let_status = 'LET: ' + unicode('%.4g' % self.let_value) + " keV/um"
        # Send a message with the text to the 2nd and 3rd statusbar sections
        pub.sendMessage('main.update_statusbar', {0:let_status,1:text, 2:value})
    ...
    def ChangeLET(self,evt):
        delta = self.mousepos - evt.GetPosition()        
        self.let_value -= delta[0]
        if self.let_value < 0:
            self.let_value = 0
        elif self.let_value > self.let_max:
            self.let_value = self.let_max
        self.Refresh()
