...
def pluginProperties():
	props = {}   
	props['name'] = 'LET Plot'                                                                                                    
	props['description'] = "Plot LET"                                                               
	props['author'] = 'Jakob Toftegaard'
	props['version'] = 0.1                                                                                                  
	props['plugin_type'] = 'main'                                                                        
	props['plugin_version'] = 1                                                                                                    
	props['min_dicom'] = ["letdata"]                                                                                                       
	props['recommended_dicom'] = ['images', 'rtss', 'rtplan', 'rtdose']
	return props

def pluginLoader(parent):
	datapath = guiutil.get_data_dir()
	userpath = os.path.join(datapath, 'plugins/LETPlot/letplot.xrc')
    	res = XmlResource(userpath)

    	panel = res.LoadPanel(parent, 'AptgLETPlotPanel')
    	panel.Init(res)
    	return panel

class AptgLETPlotPanel(wx.Panel):
	def __init__(self):
		pre = wx.PrePanel()
		self.PostCreate(pre)
	def Init(self,res):
		...
		self.preferences = [
		    {'Drawing Settings':
			    [{'name':'Plot Method',
			'type':'choice',
			'values':['IsoDose','Waterfall'],
			'default':'IsoDose',
			'callback':'AptgLETPlotPanel.drawingprefs.plot_style'},
		...
		pub.subscribe(self.OnUpdateRawData, 'patient.updated.raw_data')
        ...
        def OnUpdateRawData(self,msg):
                if msg.data.has_key('letdata'):
                    self.letdata = msg.data['letdata']
                    self.let_max = self.letdata.get_max()
                    self.let_level = self.let_max/2
	...
	def DrawIsodose(self, isodose, gc, isodosegen):
		...
		if len(contours):
		    ...
		    gc.DrawPath(path)
        def DrawIsoLET(self, let_level, gc,isoletgen):
            color = wx.Colour(160, 32,
		        240, int(self.isodose_fill_opacity*255/100))
            contours = isoletgen.trace(let_level)
            if len(contours):
	...
	def OnPaint(self, evt):
		...
		if len(self.images):
		    ...
		    if len(self.isodoses):
	            ...
	            i_z = self.letdata.pos_to_indices([0,0,float(self.z)])[2]
	            grid = self.letdata.cube[i_z]
		    x, y = np.meshgrid(
		           np.arange(grid.shape[1]), np.arange(grid.shape[0]))
		            # Instantiate the isodose generator for this slice
		    isoletgen = cntr.Cntr(x, y, grid)

                    self.DrawIsoLET(self.let_level,gc,isoletgen)
	...
	def OnUpdatePositionValues(self, evt=None):
		...
		if ((0 <= xpos < len(self.structurepixlut[0])) and
		    ...
		    # pixel, if the dose has been loaded
                    if hasattr(self,"letdata") is True:
                        pos = [self.structurepixlut[0][xpos],self.structurepixlut[1][ypos],float(self.z)]
                        let = self.letdata.get_value_at_pos(pos)
                        value = value + " / LET: " + \
		                    unicode('%.4g' % let) + " keV/um / "
	...
	def OnLeftIsDown(self, evt):
		...
		self.Refresh()
        def change_let_level(self,evt):
                delta = self.mousepos - evt.GetPosition()
		self.mousepos = evt.GetPosition()
                self.let_level -= delta[0]
                if self.let_level < 0:
                    self.let_level = 0
                elif self.let_level >= self.let_max:
                    self.let_level = self.let_max
                text = "LET level: " + \
		                    unicode('%.4g' % self.let_level) + " keV/um"
                self.Refresh()
                pub.sendMessage('main.update_statusbar', {0:text})
