...
class DensityProjections:
    ...
    def calculate_quality_list(self,voi,gantry,couch,calculate_from=0,stepsize=1.0,avoid=[],gradient = True):
        ...
        for gantry_angle in gantry:
            p = Process(target=self.calculate_angle_quality_thread,args=(voi,gantry_angle,couch,calculate_from,stepsize,q,avoid,voi_cube,gradient))
            p.start()
            p.deamon = True
            process.append(p)
            if len(process) > 2:
    ...
    def calculate_best_angles(self,voi,fields,min_dist=20,gantry_limits=[-90,90],couch_limits=[0,359],avoid=[]):
        grid = self.calculate_quality_list(voi,range(gantry_limits[0],gantry_limits[1],20),range(couch_limits[0],couch_limits[1],20),avoid=avoid)
    ...
    def optimize_angle(self,voi,couch,gantry,margin,iteration,avoid=[]):
        if iteration is 0:
            return [gantry,couch]
        grid = self.calculate_quality_list(voi,numpy.linspace(gantry,gantry+margin,3),numpy.linspace(couch,couch+margin,3),avoid=avoid)
    ...
    def calculate_angle_quality_thread(self,voi,gantry,couch,calculate_from=0,stepsize=1.0,q=None,avoid=[],voi_cube=None,gradient=True):
        os.nice(1)
        for couch_angle in couch:
            qual =  self.calculate_angle_quality(voi,gantry,couch_angle,calculate_from,stepsize,avoid,voi_cube,gradient)
            q.put({"couch":couch_angle,"gantry":gantry,"data":qual})
    def calculate_angle_quality(self,voi,gantry,couch,calculate_from=0,stepsize=1.0,avoid=[],voi_cube=None,gradient = True):
    ...
    def calculate_projection(self,voi,gantry,couch,calculate_from=0,stepsize=1.0):
        ...
        window_size = numpy.array([((sin(couch)*size[1])**2+(cos(couch)*size[2])**2)**0.5,((sin(gantry)*size[0])**2+(cos(gantry)*size[1])**2+(sin(couch)*size[2])**2)**0.5])*2
        ...
        data = pytriplib.calculate_wepl(self.cube.cube,numpy.array(start),numpy.array(basis)*step_length,dimension,numpy.array([self.cube.pixel_size,self.cube.pixel_size,self.cube.slice_distance]))
        data *= step_length
