...
def test_point(kind):
    ...
    assert np.all(np.isclose(interp_1d(x=(1, 1, 1)), (2, 2, 2)))
    assert np.all(np.isclose(interp_1d(x=(0, 1, 2)), (2, 2, 2)))  # some values outside domain
    assert np.all(np.isclose(interp_1d(np.array((1, 1, 1))), (2, 2, 2)))  # numpy array
    # assert interp_1d(()) == ()  # empty table

    interp_2d = RegularInterpolator(x=(1.,), y=(2.,), z=(3.,), kind=kind)
    assert interp_2d(1, 2) == 3
    assert interp_2d(5, 0) == 3  # value outside domain
    assert np.all(np.isclose(interp_2d((1, 1, 1), (2, 2, 2)), (3, 3, 3)))
    test_y = RegularInterpolator.eval(x=(1, 1, 1), y=(2, 2, 2), xp=(1.,), yp=(2.,), zp=(3.,), kind=kind)
    #assert np.all(np.isclose((test_y, (3, 3, 3))))
    assert np.all(np.isclose(interp_2d((0, 1, 2), (0, 1, 2)), (3, 3, 3)))  # some values outside domain
    assert np.all(np.isclose(interp_2d(np.array((1, 1, 1)), np.array((2, 2, 2))), (3, 3, 3)))  # numpy array
