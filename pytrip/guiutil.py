...
class PlotUtil:
    ...
    def pixel_to_pos(self,pixel):
        if self.plot_plan == "Transversal":
            ...
            pos = [(self.ctx.dimy-pixel[0])*self.ctx.pixel_size,(self.ctx.dimz-pixel[1])*self.ctx.slice_distance]
        elif self.plot_plan == "Coronal":
    ...
    def plot(self,idx):
        ...
        if not hasattr(self,"fig_ct"):
            self.fig_ct = self.figure.imshow(ct_data,cmap=plt.get_cmap("gray"),vmin=self.contrast[0],vmax=self.contrast[1],aspect=self.aspect)
        else:
    ...
    def plot_text(self,idx):
        ...
        if self.plot_plan == "Transversal":
            ...
            slice_dist = self.ctx.pixel_size
        self.figure.text(offset[0],offset[1]+3.0/self.zoom*100,"Slice #: %d/%d\nSlice Position: %.1f mm"%(idx,slices,idx*slice_dist),color="white",va="top", fontsize=8)
        self.figure.text(offset[0]+width/self.zoom*100,offset[1]+3.0/self.zoom*100,"W / L: %d / %d"%(self.contrast[1],self.contrast[0]),ha="right",color="white",va="top",fontsize=8)

        self.figure.text(offset[0],offset[1]+(height-5)/self.zoom*100,self.plot_plan,color="white",va="bottom",fontsize=8)
