...
class TripExecuter(object):
    ...
    def analyse_cube(self):
        ...
        for i,field in enumerate(p1["fields"]):
            ...
            basis = basis[0]
            basis = np.array([basis[0]/cube1.pixel_size,basis[1]/cube1.pixel_size,basis[2]/cube1.slice_distance])
    ...
    def create_exec_load_data_files(self,projectile):
        ...
        if not self.plan.get_target_tissue_type() == "":
            ...
            if not self.plan.get_res_tissue_type() == "" and not self.plan.get_res_tissue_type() == self.plan.get_target_tissue_type():
                rbe = self.rbe.get_rbe_by_name(self.plan.get_target_tissue_type())
                output.append("rbe %s /read"%(rbe.get_path()))
    ...
    def split_fields(self,proj):
        if not self.split_proj_key in self.projectiles.keys():
            return
        ...
        if len(p["fields"]) == 2 :
            temp.cube = (temp.cube<self.projectiles[proj]["target_dos"].cube)*self.projectiles[proj]["target_dos"].cube + (temp.cube>self.projectiles[proj]["target_dos"].cube)*temp.cube
            ...
            temp.cube = temp.cube*(self.target_dos.cube>0)
            ...
            self.projectiles[field1.get_projectile()+str(1)] = {"target_dos":d1,"fields":[field1],"name":field1.get_projectile()+str(1),"projectile":field1.get_projectile()}
            self.projectiles[field2.get_projectile()+str(2)] = {"target_dos":d2,"fields":[field2],"name":field2.get_projectile()+str(2),"projectile":field2.get_projectile()}
    ...
    def post_process(self):
        ...
        if not bio_dose is None:
            bio_dose.write(out_path + ".bio.dos")
        if not phys_dose is None:
            phys_dose.write(out_path + ".phys.dos")
        if not dose_mean_let is None:

            if self.mult_proj:
                dose_mean_let = dose_mean_let /phys_dose
        ...
        if self.mult_proj:
            ...
            for field in plan.fields:
                if field.get_projectile() not in self.projectiles.keys():
                    self.projectiles[field.get_projectile()] = {"target_dos":DosCube(self.images),"fields":[field],"name":field.get_projectile(),"projectile":field.get_projectile()}
                else:
            ...
            for i,voi in enumerate(self.targets):
                ...
                for projectile,data in self.projectiles.items():
                    ...
                    proj_dose_level = int(voi.get_dose()/self.target_dose*dose_percent*10)
                    if self.projectile_dose_level[projectile] < proj_dose_level:
                        self.projectile_dose_level[projectile] = proj_dose_level
                    if proj_dose_level == 0:
                        proj_dose_level = -1
                    if i == 0:
                        data["target_dos"] = temp*proj_dose_level
                    else:
                        data["target_dos"].merge_zero(temp*proj_dose_level)
                if i == 0:
    ...
    def run_trip_remote(self):
        ...
        commands = ["tar -zxvf temp.tar.gz","cd %s;bash run"%self.folder_name,"tar -zcvf temp.tar.gz %s"%self.folder_name,"rm -r %s"%self.folder_name]
        for cmd in commands:
            self.log(cmd)
    ...
    def create_trip_exec_mult_proj(self,projectile,no_output = False,last=True,first=True):
        ...
        if not no_output:
            if hasattr(self,"rest_dose"):
                dosecube.cube = np.array((self.rest_dose.cube>=dosecube.cube)*dosecube.cube + (self.rest_dose.cube<dosecube.cube)*self.rest_dose.cube,dtype=np.int16)
