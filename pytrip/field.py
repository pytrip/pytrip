...
class SubField:
    ...
    def get_size(self):
        ...
        lateral[0] = lateral[0]-beamsize
        lateral[1] = lateral[1]+beamsize
        lateral[2] = lateral[2]-beamsize
        lateral[3] = lateral[3]+beamsize
    ...
    def get_merge_raster_points(self,size):
        ...
        #~ dist = np.array([[(x[0]-y[0])**2+(x[1]-y[1])**2 for y in points] for x in points])
        #~ 
        #~ particles = np.sum(np.array([points[:,2]]*len(points))*1/((2*pi*sigma**2)**0.5)*np.exp(-dist/(2*sigma**2)),axis=1)
        #~ self.points = [[point[0],point[1],p] for point,p in zip(points,particles)]
    ...
    def get_raster_matrixs(self,size):
        ...
        margin = 5
        mat = [[[x,y,0] for x in np.linspace(size[0]-margin*step[0],size[1]+margin*step[0],(size[1]-size[0])/step[0]+1+2*margin)] for y in np.linspace(size[2]-margin*step[1],size[3]+margin*step[1],(size[3]-size[2])/step[1]+1+2*margin)]
        for p in points:
            mat[int(p[1]/step[1]-size[2]/step[1])+margin][int(p[0]/step[0]-size[0]/step[0])+margin] = p
        return mat
    ...
    def get_subfield_cube(self,resolution=0.5):
        ...
        size = self.get_size()
        #~ doesn't allow minimum values greatet than zero
        ...
        for point in raster:
            ...
            start = np.array([raster_center[0]-(raster_dim[0]+1)/2,raster_center[1]-(raster_dim[1]+1)/2,0],dtype=int)
            end = np.array([raster_center[0]+(raster_dim[0]-1)/2,raster_center[1]+(raster_dim[1]-1)/2,raster_dim[2]],dtype=int)
