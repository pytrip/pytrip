...
"""
from pytrip.rst import *
from pytrip.ddd import *
from pytrip.res.point import *
from math import *
import numpy as np
from pytrip.res.utils import *
...
class SubField:
    ...
    def get_raster_matrixs(self, size):
        ...
        margin = 5
        mat = [[[x, y, 0] for x in np.linspace(size[0] - margin * step[0], size[1] + margin * step[0],
                                               (size[1] - size[0]) / step[0] + 1 + 2 * margin)] for y in
               np.linspace(size[2] - margin * step[1], size[3] + margin * step[1],
