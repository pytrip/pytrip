...
class SubField:
    ...
    def get_lateral(self, resolution=0.5):
        ...
        lateral = 1 / ((2 * pi * sigma**2)**0.5) * np.exp(-(r**2) /
                                                          (2 * sigma**2))
        tot_lat = np.zeros((2 * dim - 1, 2 * dim - 1))

        tot_lat[dim - 1:2 * dim - 1, dim - 1:2 * dim - 1] = lateral
        tot_lat[dim - 1:2 * dim - 1, 0:dim - 1] = np.rot90(lateral,
                                                           3)[:, 0:dim - 1]
        tot_lat[0:dim - 1, 0:dim - 1] = np.rot90(lateral, 2)[0:dim - 1,
                                                             0:dim - 1]
        tot_lat[0:dim - 1,
                dim - 1:2 * dim - 1] = np.rot90(lateral)[0:dim - 1, :]
    ...
    def get_raster_matrixs(self, size):
        points = sorted(self.submachine.get_raster_points(),
                        key=cmp_to_key(compare_raster_point))
        step = self.submachine.stepsize
        margin = 5
        mat = [[[x, y, 0]
                for x in np.linspace(size[0] - margin * step[0], size[1] +
                                     margin * step[0], (size[1] - size[0]) /
                                     step[0] + 1 + 2 * margin)]
               for y in np.linspace(size[2] - margin * step[1], size[3] +
                                    margin * step[1], (size[3] - size[2]) /
                                    step[1] + 1 + 2 * margin)]
        for p in points:
    ...
    def get_subfield_cube(self, resolution=0.5):
        ...
        field = np.zeros(
            (dim[5] + dim[4], dim[3] + dim[2] - 1, dim[1] + dim[0] - 1))
        lateral = self.get_lateral(resolution)
        ddd = self.ddd.get_ddd_by_energy(
            self.energy, np.linspace(0, (dim[5]) * resolution, dim[5] * 10))
        ...
        for point in raster:
            ...
            start = np.array([
                raster_center[0] - (raster_dim[0] + 1) / 2, raster_center[1] -
                (raster_dim[1] + 1) / 2, 0
            ],
                             dtype=int)
            end = np.array([
                raster_center[0] + (raster_dim[0] - 1) / 2, raster_center[1] +
                (raster_dim[1] - 1) / 2, raster_dim[2]
            ],
                           dtype=int)
            field[start[2]:end[2], start[1]:end[1],
                  start[0]:end[0]] += point[2] * raster_field
        return field, zero
